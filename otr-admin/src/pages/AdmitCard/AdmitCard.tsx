...
import { GetCategories, uploadAdmitCard } from "../../services/apiHelpers";
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-2">
            <button
              ...
              className={`px-4 py-2 text-white rounded-lg transition ${signatureFile
...
const AdmitCard: React.FC = () => {
  const [jobCategoryId, setJobCategoryId] = useState<any | null>(null); // ✅ store ID
  const [jobTitle, setJobTitle] = useState("");
  const [signatureData, setSignatureData] = useState("");
  const [pdfFile, setPdfFile] = useState<File | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [categories, setCategories] = useState<{ id: number; jobCategory: string }[]>([]);
      ...
      try {
        ...
        if (response?.status === 200 && Array.isArray(response.data)) {
          ...
          if (response.data.length > 0) {
            setJobCategoryId(response.data[0].id);
          }
        } else {
  ...
  const handleGenerate = async () => {
    try {
      const payload = {
        jobCategoryId, // ✅ this is numeric id
        jobTitle,
        ...
        <div className="grid grid-cols-1 gap-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="relative w-2/3">
              {loading ? (
                <div className="p-2 text-gray-400 italic">Loading...</div>
              ) : (
                <select
                  value={jobCategoryId ?? ""}
                  onChange={(e) => setJobCategoryId(Number(e.target.value))} // ✅ store ID
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="w-2/3 flex items-center space-x-3">
              <div
                className={`flex-1 p-1 border rounded-lg h-10 flex items-center justify-center cursor-pointer ${signatureData
                    ? "border-green-500"
